...
def graficos(canvas, y, *args):
    drawings = [drawing for drawing in args if drawing is not None]
    dy = max(drawing.height for drawing in drawings)
    dx = (A4[0] - sum(drawing.width for drawing in drawings)) / (len(drawings)+1)
...
def titulo(canvas, y, titulo, fontSize=12, subrayado=None):
